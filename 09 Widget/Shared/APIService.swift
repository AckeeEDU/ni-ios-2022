...
public final class TestAPIService: APIServicing {
    public var feedPublisherBody: () -> AnyPublisher<[Post], Error> = {
    ...
    public var commentsPublisherBody: (String) -> AnyPublisher<[Comment], Error> = { _ in
        Just([]).setFailureType(to: Error.self).eraseToAnyPublisher()
    }
